...
Here is an implementation with $\epsilon=0$:
```{r matern_defn}
...
temporal.coords <- sort(runif(k))
geo.dist <- fields::rdist(spatial.coords)
time.dist <- fields::rdist(temporal.coords)
layout(t(1:2))
image(geo.dist,main="geographic distance")
image(time.dist,main="time difference")
```

```{r plot_matern, fig.width=3*fig.dim}
layout(t(1:3))
plot.fn <- function (alpha,nu,zeta) {
    covmat <- spatiotemporal.covariance(geo.dist,time.dist,alpha=alpha,nu=nu,zeta=zeta)
    image(covmat)
    mtext( substitute( group( "(", list( alpha == alphaval, nu == nuval, zeta == zetaval ), ")" ), list(alphaval=alpha, nuval=nu, zetaval=zeta) ), side=3 )
    plot( geo.dist, as.vector(covmat), xlab="geographic dist", ylab="covariance" )
    plot( time.dist, as.vector(covmat), xlab="geographic dist", ylab="covariance" )
}

plot.fn( alpha=1, nu=1, zeta=1 )
plot.fn( alpha=.1, nu=1, zeta=1 )
plot.fn( alpha=1, nu=.1, zeta=1 )
plot.fn( alpha=1, nu=1, zeta=.1 )
```
