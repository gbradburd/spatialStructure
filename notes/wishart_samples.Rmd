...
pick allele frequencies $F=\mu+X$ by truncating a sample from $N(\mu,\mu(1-\mu)V)$,
...
\end{aligned}
$$
If $\E[F_i|\mu] = \mu$ and $\cov[F_i,F_j|\mu] = \mu(1-\mu)V_{ij}$, 
as would be the case if we ignored truncation, then
$$
\begin{aligned}
    \cov[F_i,F_j] 
    &= 
    ...
    &=
    \E[\mu(1-\mu)] V_{ij} + \var[\mu] \\
    % &=
    % \E[\mu](1-\E[\mu]) V_{ij} + (1-V_{ij})\var[\mu] ,
\end{aligned}
$$
and also
$$
\begin{aligned}
    \E[ F_i (1-F_i) ]
    &= 
    \E[ F_i ] - \E[ F_i ]^2 - \var[ F_i ] \\
    &=
    \E[ \mu ] ( 1- \E[\mu] ) - ( \E[\mu(1-\mu)] V_{ii} + \var[\mu] ) \\
    &=
    \E[ \mu (1-\mu) ] (1-V_{ii})
\end{aligned}
$$
...
\begin{aligned}
    \cov[P_i,P_j] 
    &= 
    \delta_{ij} \frac{1}{n_i} \E[ \mu (1-\mu) ] (1-V_{ii})
        + \var[\mu] + \E[\mu(1-\mu)] V_{ij}  
\end{aligned}
$$
This suggests the parametric form
$$
\Omega_{ij} = \zeta \left( \delta_{ij} \eta_i + V_{ij} \right) + \gamma .
$$
Note that $\zeta$ is confounded with $\eta$ and $V$, so it should probably be left out.
...
```{r stan_posterior_alphas, fig.height=2*fig.dim, cache=TRUE}
...
```{r stan_posterior_mean, cache=TRUE}
ut <- upper.tri(dmat,diag=TRUE)
...
```{r stan_posterior_cov, cache=TRUE}
