...
update.w.i<-function(i, data.list, parameter.list, model.options, mcmc.quantities){
	...
	if( !(new.w.1<0 | new.w.1>1)  &  !(new.w.2<0 | new.w.2>1) )  {
		...
		inverse.prime <- double.sherman_r(parameter.list$inverse, u, v,i )
		determinant.prime <- parameter.list$determinant
		...
		old.determinant<-parameter.list$determinant
		   #this function is current in Sherman in sandbox
...
MCMC <- function(	data,
	...
	update.w.i(1, data.list, super.list$parameter.list, model.options, super.list$mcmc.quantities)
