...
alpha0.1 <- 1; alpha1.1 <- 2; alpha2.1 <- 1.5; 
my.covar.1<- alpha0.1 * exp(-(alpha1.1 * my.dists)^alpha2.1)
alpha0.2 <- 0.5; alpha1.2 <- 1.2; alpha2.2 <- 0.5; 
my.covar.2<- alpha0.2 * exp(-(alpha1.2 * my.dists)^alpha2.2)

my.ws <- rbeta(N,0.9,0.9)
my.covar <- (my.ws %*% t(my.ws)) * my.covar.1  +  ((1-my.ws) %*% t(1-my.ws)) * my.covar.2

nuggets <- rexp(N,rate=10)
diag(my.covar) <- diag(my.covar) + nuggets

plot(c(my.dists),c(my.covar))
mv.data <- MASS::mvrnorm(n = L, mu=rep(0,N), Sigma = my.covar)
