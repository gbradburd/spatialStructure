...
double.sherman_r <- function(Ap, u, v) {
	Sherman.correction<-(Ap %*% u %*% t(v) %*% Ap)/drop(1 + t(v) %*% Ap %*% u)
	recover()
  Ap - Sherman.correction -  t(Sherman.correction)
}
...
res <- microbenchmark(cpp_dsherman={ use_doublesherman(ms, u, v) },
