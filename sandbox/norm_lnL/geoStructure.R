...
get.std.error <- function(freqs,breaks=NULL){
	n.loci <- ncol(freqs)
	if(is.null(breaks)){
		breaks <- n.loci/100
	}
	blocks <- as.numeric(cut(1:n.loci,breaks=breaks))
	block.covs <- lapply(1:breaks,
							function(i){
								cov(t(freqs[,blocks==i]),use="pairwise.complete.obs")
							})
	sample.cov <- cov(t(freqs),use="pairwise.complete.obs")
	w.dif <- Reduce("+",lapply(block.covs,function(w){(sample.cov - w[1,2])^2}))
	std.err.mat <- sqrt(w.dif/(breaks*(breaks-1)))
	return(std.err.mat)
}

standardize.freqs <- function(freqs,sample.sizes,breaks=NULL){
	...
	std.errs <- get.std.error(freqs,breaks)
...
make.data.block <- function(K,std.freq.list,D,coords,sample.sizes,prefix,spatial){
	data.block <- list( "K" = K,
						"N" = nrow(D),
						"obsSigma" = std.freq.list$sample.cov,
						"geoDist" = D,
						"stdErrs" = std.freq.list$std.errs,
...
get.ML.cluster.params <- function(model.fit,data.block,cluster,n.clusters,n.iter=NULL){
	cluster.params <- list()
	if(data.block$spatial){
...
get.ML.cluster.params.list <- function(model.fit,data.block,n.iter=NULL){
	cluster.params <- setNames(
						lapply(1:data.block$K,
									function(i){
										get.ML.cluster.params(model.fit,data.block,i,data.block$K,n.iter)
									}),
						paste("Cluster",1:data.block$K,sep="_"))
	return(cluster.params)
}

get.geoStructure.ML.results <- function(model.fit,data.block,n.iter=NULL){
	point <- list("admix.proportions" = get.ML.admix.props(model.fit,data.block),
				  "nuggets" = get.ML.nuggets(model.fit,data.block),
				  "cluster.params" = get.ML.cluster.params.list(model.fit,data.block,n.iter),
...
get.cluster.cov <- function(cluster.params,data.block,n.iter=NULL){
